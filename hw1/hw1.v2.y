...
#define MAX_NUMBER_OF_MACROS 1000
#define MAX_NUMBER_OF_MACRO_ARGS 100
#define MAX_ARG_INDEX 10

////////   DEBUG MACROS  /////////
// #define DEBUG 0
#define FORMAT_OUTPUT 1
#ifdef DEBUG
#define CONCATDEBUG 1
#define SHOW_MACRO 1
...
void createMacro(char* name, struct token* args, struct token* body);
...
MacroDefExpression: KW_HD ID BL1 ArgumentList BR1 BL1 Expression BR1{ 
                           ...
                           createMacro($2->str, $4, concat(macroBody));
...
void createList(int macroNumber){
    ...
    struct token** lists = (struct token**) malloc(sizeof(struct token*)*(num+3));
    cur = listCopy;
...
struct token* applyMacros(struct token* macroCall, int semicolon){
   ...
   while(cur!=NULL){
    ...
    // replace the corresponding strings in the list
//    int loc = -1;
//    struct token* macroBodyList = copyList(macroArr[macroNumber].body);
//    cur = macroBodyList;
//    while(cur->nextToken!=NULL){
//        if(MACRODEBUG) { printf("macrobody token : %s\n", cur->str); }
//        loc=isPresent(macroArr[macroNumber].arguments, cur->str);
//        if(MACRODEBUG) { printf("loc %d\n", loc); fflush(stdout); }
//        if(loc!=-1){
//            // free(cur->str); // THIS IS GIVING SEGFAULT
//            cur->str = strdup(callArgs[loc]);
//        }
//        cur = cur->nextToken;
//    }
    int listNo = 0;
